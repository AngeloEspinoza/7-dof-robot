...
#define MAX_NODES 100 // Maximum number allowed of nodes to visit

int count_nodes = 0;

float **coordinates_to_follow(void)
{
	float auxiliar[MAX_NODES][2];

	/* Reding .txt file */
	std::ifstream input_file;
	input_file.open("/home/angelo/Desktop/coordinates.txt");
	...
	float  **coordinates = new float*[COLUMNS];

	for (int i = 0; i < count_nodes; ++i)
	{
		coordinates[i] = new float[count_nodes];
